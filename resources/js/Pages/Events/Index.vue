...
<template>
  <div class="min-h-screen bg-gray-50" style="font-family: Arial, sans-serif;">
    ...
    <main class="max-w-6xl mx-auto px-4 sm:px-6 py-10">
      ...
      <div v-if="featured"
        ...
        <div class="absolute bottom-0 left-0 right-0 p-8 z-20">
          <span class="inline-block text-xs font-bold bg-yellow-400 text-yellow-900 px-3 py-1 rounded-full mb-3">
            ⭐ Featured Event
          </span>
        ...
        <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          <a v-for="event in events" :key="event.id"
            ...
            <div class="p-4">
              ...
              <p class="text-xs text-gray-500 mb-1">📅 {{ event.starts_at }}</p>
              <p v-if="event.venue" class="text-xs text-gray-400 mb-3">📍 {{ event.venue }}</p>

              <div class="flex items-center justify-between mt-auto pt-3 border-t border-gray-100">
                <span class="text-sm font-bold text-gray-900">
                  {{ currency(event.min_price) }}
                </span>
    ...
    <footer class="text-center text-xs text-gray-400 py-8">
      {{ app.name }} &mdash; Powered by Nexus
    </footer>
  </div>
</template>
