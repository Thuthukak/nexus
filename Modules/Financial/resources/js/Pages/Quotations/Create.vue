...
const newCustomer     = ref({ company_name: '', contact_name: '', email: '', phone: '' })
...
async function createCustomer() {
  ...
  try {
    ...
    newCustomer.value     = { company_name: '', contact_name: '', email: '', phone: '' }
  } catch (err) {
...
function applyNetTerm(termKey) {
  ...
  if (term && term.days !== null) {
    const d = new Date(Date.now() + term.days * 864e5)
    form.valid_until = d.toISOString().split('T')[0]
  }
...
const form = useForm({
  ...
  issue_date:  props.quotation?.issue_date  ?? today,
  valid_until: props.quotation?.valid_until ?? '',
  net_terms:   props.quotation?.net_terms   ?? 'valid_30',
...
function submit() {
  ...
  if (isEditing.value) {
    form.transform(() => payload).put(`/financial/quotations/${props.quotation.id}`)
  } else {
    form.transform(() => payload).post('/financial/quotations')
  }
}
</script>
  ...
  <div class="max-w-4xl">
    ...
    <form @submit.prevent="submit" class="space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <Transition enter-active-class="transition-all duration-200 ease-out"
            ...
            <div v-if="showNewCustomer"
                 class="sm:col-span-2 rounded-xl border-2 border-primary/20 bg-primary/5 p-5">
              ...
              <div class="grid grid-cols-2 gap-3">
                <div class="col-span-2 flex flex-col gap-1">
                  ...
                  <input v-model="newCustomer.company_name" type="text"
                ...
                <div class="flex flex-col gap-1">
                  ...
                  <input v-model="newCustomer.contact_name" type="text"
                         class="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-background text-app-text text-sm focus:outline-none focus:ring-2 focus:ring-primary/50" />
                </div>
                <div class="flex flex-col gap-1">
              ...
              <div class="mt-3 flex justify-end gap-2">
                <button type="button" @click="showNewCustomer = false"
                        class="px-3 py-1.5 text-sm text-app-text/60">Cancel</button>
          ...
          <!-- Validity terms -->
          <div class="flex flex-col gap-1">
            <label class="text-sm font-medium text-app-text">Valid For</label>
            <div class="flex gap-2">
              <select @change="applyNetTerm($event.target.value)"
                      :value="selectedTerm"
                      class="flex-1 px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-background text-app-text text-sm focus:outline-none focus:ring-2 focus:ring-primary/50">
                <option v-for="(term, key) in netTerms" :key="key" :value="key">{{ term.label }}</option>
              </select>
            ...
            <p v-if="form.errors.valid_until" class="text-xs text-red-500">{{ form.errors.valid_until }}</p>
          </div>
        </div>
      </div>
          ...
          <div v-for="(line, i) in form.lines" :key="i" class="space-y-2">
            ...
            <Transition enter-active-class="transition-all duration-200 ease-out"
              ...
              <div v-if="showNewProduct.index === i"
                ...
                <div class="flex items-center justify-between mb-3">
                  <p class="text-sm font-semibold text-app-text">New Product</p>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  ...
                  <div class="flex flex-col gap-1">
                    ...
                    <input v-model="newProduct.unit" type="text" placeholder="hour, item…"
                           class="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-background text-app-text text-sm focus:outline-none focus:ring-2 focus:ring-primary/50" />
                  </div>
                </div>
                <div class="mt-3 flex justify-end gap-2">
      ...
      <div class="flex items-center justify-between">
        <a href="/financial/quotations" class="px-4 py-2 text-sm text-app-text/60 hover:text-app-text">Cancel</a>
